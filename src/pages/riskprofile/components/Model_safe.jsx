...
const ModelDemo = () => {
  ...
  const [isGenesOpen, setIsGenesOpen] = useState(false); // New state for dropdown
  ...
  const existingPatients = [
    { id: 'TCGA-A1-A0SB', age: 62.5, node: 'Positive' },
    { id: 'TCGA-A2-A0T2', age: 45.0, node: 'Negative' },
    { id: 'TCGA-B3-A123', age: 71.2, node: 'Positive' },
  ];
  ...
  const handlePatientSelect = (e) => {
    ...
    if (patient) {
      setFormData(prev => ({ ...prev, age: patient.age, nodeStatus: patient.node }));
    }
  };
  ...
  const handleGeneChange = (geneKey, value) => {
    setFormData(prev => ({
      ...prev,
      genes: { ...prev.genes, [geneKey]: parseFloat(value) }
    }));
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-start mb-8 border-b border-slate-800 pb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-emerald-500 text-xs font-medium uppercase tracking-wider">System Ready</span>
          </div>
        </div>
          ...
          <div className="p-6 md:p-8 space-y-8">
              ...
              >
                <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                        ...
                        <span className="text-xs text-slate-500">Configure 30-gene expression panel (z-scored)</span>
                    </div>
                </div>
              ...
              {isGenesOpen && (
                <div className="mt-4 bg-[#0f172a] rounded-xl border border-slate-700 p-4 animate-fade-in">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                    {Object.keys(formData.genes).map((gene) => (
                      <div key={gene}>
            ...
            <div className="flex gap-4 pt-4 border-t border-slate-800">
              <button
                ...
                disabled={isLoading || (mode === 'existing' && !selectedPid)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className={cardClass}>
                        ...
                        <div className="flex justify-between items-start mb-3">
                            <div>
                                <h4 className="text-slate-200 font-bold">Relative Risk (Cox Hazard)</h4>
  ...
  <div className="flex items-center gap-2 mb-4">
    ...
    <h2 className="text-xl font-bold text-white">
      Survival Time Estimates (model-based)
    </h2>
  </div>

  {/* Description Text - Wrapped in a div for proper spacing from the grid below */}
  <div className="mb-8 text-sm text-slate-400">
    <p className="mb-2">
      Time measured from diagnosis or study start until an event in the
      dataset. These are statistical summaries of modeled survival curves,
    ...
    <ul className="list-disc list-inside text-xs text-slate-500 space-y-1 ml-2">
      <li>
        Values summarize expected survival behavior across similar patients,
        not a fixed timeline.
      </li>
      <li>
        "Not reached" means the modeled survival probability never dropped
        below 50% within the observed follow-up window.
      </li>
    </ul>
  </div>
  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div className="bg-[#0f172a] rounded-lg p-5 border border-slate-800">
      ...
      <div className="text-3xl font-bold text-white mb-3">
        {results.estimates.medianCox
          ? `${results.estimates.medianCox} days`
          : "Not reached"}
      </div>
      <p className="text-xs text-slate-400 leading-relaxed">
        Median survival time inferred from the Cox survival curve. Reported
        as “Not reached” if survival probability does not fall below 50%
    ...
    <div className="bg-[#0f172a] rounded-lg p-5 border border-slate-800">
      ...
      <div className="text-3xl font-bold text-white mb-3">
        {results.estimates.medianRsf
          ? `${results.estimates.medianRsf} days`
          : "Not reached"}
      </div>
      <p className="text-xs text-slate-400 leading-relaxed">
        Median survival time inferred from the Random Survival Forest
        survival curve. Represents the time at which modeled survival
        probability falls below 50%.
      </p>
    </div>

    {/* Consensus */}
    <div className="bg-[#0f172a] rounded-lg p-5 border border-slate-800">
      <h4 className="text-purple-400 font-bold text-lg mb-1">
        Consensus Median
      </h4>
      <div className="text-3xl font-bold text-white mb-3">
        {results.estimates.consensus
          ? `${results.estimates.consensus} days`
          : "Not reached"}
      </div>
      <p className="text-xs text-slate-400 leading-relaxed">
        Median survival estimate when both models report a similar value.
        Shown for comparison only; no single value is treated as absolute
        truth.
      </p>
    </div>
  </div>
             ...
             <section className="bg-[#1e293b] rounded-xl border border-slate-700/50 p-6 h-[400px]">
                ...
                <LineChart data={results.curveData} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
                    ...
                    <XAxis 
                        ...
                        label={{ value: 'Time (days)', position: 'insideBottom', offset: -5, fill: '#64748b' }} 
                    />
                    <YAxis 
                        stroke="#94a3b8"
                        tick={{fill: '#94a3b8', fontSize: 12}}
